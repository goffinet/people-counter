...
Configuration :
  ...
  DOOR_THRESHOLD  : sensibilité détection porte (0-100)
...
#   docker compose exec app kill -USR1 $(cat /data/app.pid)
# ---------------------------------------------------------------------------
...
def _on_new_sample(sink):
    ...
    # Recapture de référence demandée via SIGUSR1
    if _reset_ref:
...
def build_pipeline():
    ...
    q_door.set_property("leaky", 2)            # drop-head : toujours la frame la plus récente
    ...
    # Probe de comptage sur le src pad du tracker
...
def main():
    ...
    # Écriture du PID pour reset_reference.py
    ...
    # Attente de la frame de référence porte
